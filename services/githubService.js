  ...
  try {
    ...
    const filteredRepos = repos.map(repo => {
      return {
        ...
        lang: repo.language,
        url: repo.html_url,
        up_at: repo.updated_at
      };
    });
    return filteredRepos;
  } catch (error) {
    console.error(`Error fetching repos for ${username}:`, error.response?.data?.message || error.message);
    return [];
  }
}
...
const getCommits = async (owner, repo) => {
  try {
    ...
    if (error.response && error.response.status === 409) {
      // repo exists but no commits / empty repository
      return [];
    }
    console.error(`Error fetching commits for ${owner}/${repo}:`, error.response?.data?.message || error.message);
    return [];
  }
}
...
const getRepoName = async (username) => {
  try {
    ...
    console.error(`Error fetching repo names for ${username}:`, error.response?.data?.message || error.message);
    return [];
  }
}
